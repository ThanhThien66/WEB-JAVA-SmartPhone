		    ...
		    <div class="content-grids">
		    		...
		    		<c:if test = "${loop.index != list.size()-1}">
         				<c:if test = "${loop.index%4 == 0}">
         					...
         					<div class="grid_1_of_4 images_1_of_4 products-info"><a href="sp?id=${sanpham.id}">
									<img style="width: 300px; height: 238px" src="/dienthoaishop/img/${sanpham.id}.png">
		    	...
		    	<c:if test = "${list.size() != 0}">
            		...
            		<c:if test = "${currentPage != 1}">
                		<a href="/dienthoaishop/search?name=${name}&page=${currentPage-1}&sort=${sort}&range=${range}&brand=${brand}&manufactor=${manufactor}">Back</a>
                	</c:if>
                	<c:if test = "${currentPage == 1}">	
        				<a class="current">1</a>
        			</c:if>
        			<c:if test = "${currentPage != 1}">	
        				<a href="/dienthoaishop/search?name=${name}&page=1&sort=${sort}&range=${range}&brand=${brand}&manufactor=${manufactor}">1</a>
        			</c:if>
        			...
        			<c:forEach var="pag" items="${pageList}" varStatus="loop">
        				...
        				<c:if test = "${currentPage != pag}">	
        					<a href="/dienthoaishop/search?name=${name}&page=${pag}&sort=${sort}&range=${range}&brand=${brand}&manufactor=${manufactor}">${pag}</a>
        				</c:if>
      				</c:forEach>

            		<c:if test = "${currentPage != totalPage}">
                		<a href="/dienthoaishop/search?name=${name}&page=${currentPage+1}&sort=${sort}&range=${range}&brand=${brand}&manufactor=${manufactor}">Next</a>	
                	</c:if>
